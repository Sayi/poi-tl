...
CAUTION: FreeMarker、Velocity文本模板中可以通过三个标签设置图片路径、宽和高：`<img src="{{path}}" width="{{width}}" height="{{height}}">`， 但是Word模板不是由简单的文本表示，所以在渲染图片、表格等元素时提供了数据模型，它们都实现了接口 `RenderData` ，比如图片数据模型 `PictureRenderData` 包含图片路径、宽、高三个属性。
...
* icon:tags[role="red"] 当前版本 1.12.2 Documentation，Apache POI5.2.2+，JDK1.8+
...
出现异常 `不支持：http://javax.xml.XMLConstants/property/accessExternalDTD` ？::
  参考link:https://github.com/Sayi/poi-tl/issues/708[issue708]。
如何通过标签指定格式化函数？::
  Spring表达式，应有尽有。
