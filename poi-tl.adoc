...
NOTE: 源码参见 link:https://github.com/Sayi/poi-tl/blob/v1.10.0/poi-tl/src/test/java/com/deepoove/poi/tl/plugin/LoopRowTableRenderPolicyTest.java[JUnit LoopRowTableRenderPolicyTest]，如果希望模板标签和循环行在同一行而不是在上一行，可以使用 `new LoopRowTableRenderPolicy(true)` 来构造插件。
