...
Apache POI不仅在文档上层封装了易用的API(文本、图片、表格、页眉、页脚、图表等)，也可以在底层直接操作文档XML结构，**poi-tl**正是一个基于Apache POI的Word模板引擎，并且拥有着让人喜悦的特性。
...
如果你仅仅希望打开一个文档或者创建一个简单的文档，那么Apache POI就能满足你的需求；但是如果你是一个需要将数据导出成Word文档的开发者，不妨试试poi-tl；你是一个搭建Word模板云服务的开发者，不妨试试poi-tl；你开发的某个软件支持导出HTML、Markdown，不妨使用poi-tl为你的软件添加导出Word的功能。
...
poi-tl是一种 "logic-less" 模板引擎，没有复杂的控制结构和变量赋值，只有标签。所有的标签都是以 `{{` 开头，以 `}}` 结尾，模板标签可以出现在任何位置，包括页眉，页脚，表格内部，文本框等等。
...
* 多系列图表数据ChartMultiSeriesRenderData
* 单系列图表数据ChartSingleSeriesRenderData
* 嵌套数据DocxRenderData
...
== 引用标签
引用标签是一种特殊位置的标签，提供了直接引用文档中的元素句柄的能力，**这个重要的特性在我们只想改变文档中某个元素极小一部分样式和属性的时候特别有用，因为其余样式和属性都可以在模板中预置好，真正的所见即所得**。

[[ref-policy-section]]
=== 图片

[example]
引用图片标签是一个文本：{{var}}，标签位置在：设置图片格式--可选文字--标题。
...
[example]
多系列图表的标签是一个文本：{{var}}，标签位置在：图表区格式--可选文字--标题。
...
新的图表系列数据会完全替换原有数据，而原有图表的样式都会被保留。
...
[example]
单系列图表的标签是一个文本：{{var}}，标签位置在：图表区格式--可选文字--标题。
...
NOTE: 针对引用图表标签提供了 `builder.addPlugin(ChartTypes, RenderPolicy)` 方法来根据图表类型注册执行函数。
