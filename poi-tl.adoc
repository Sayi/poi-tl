...
--
.output
这是一份非常专业的Swagger Word文档，样式优雅且有着清晰完整的文档结构，接口需要循环展示，接口的参数需要循环展示，接口的返回值需要循环展示，数据类型支持锚点到具体的模型。点击link:example/example_swagger_output.png[查看大图 , window=_blank]，下载最终生成的文件link:example/swagger.docx[swagger.docx]
