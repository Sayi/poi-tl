...
|icon:check[role="green"]标签语法 |支持自定义标签前后缀
...
--
.output
这是一份非常专业的Swagger Word文档，样式优雅且有着清晰完整的文档结构，接口需要循环展示，接口的参数需要循环展示，接口的返回值需要循环展示，数据类型支持锚点到具体的模型。下载最终生成的文件link:example/swagger.docx[swagger.docx]
...
--
.template
使用区块对标签完成所有循环功能，可以完美的支持有序和多级列表，表格使用 `HackLoopTableRenderPolicy` 插件的约定，可以非常方便的完成参数、返回值和数据模型表格的渲染，使用Spring表达式来支持丰富的条件判断。
...
没错，一起都是如此简洁：简洁的代码 ，简洁的模板，甚至生成的文档都看起来那么简洁，愿一切如你所愿。
...
有什么感谢的心里话？::
  我感谢所有对poi-tl有过支持的人，特别感谢下那个打赏给我一天工资的朋友，是你让我知道我的薪水是多么的卑微(开个玩笑)。