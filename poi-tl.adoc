...
== Gradle 
[source, groovy]
compile group: 'com.deepoove', name: 'poi-tl', version: '1.4.0'
...
poi-tl的一个核心优势是 *数据模型与样式的分离*，同样的数据模型可以用来渲染各种不同样式的模板。
...
数据源可以是一个Map，其中Key是标签名称；也可以是一个JavaBean，其中field名称是标签名称，也可以通过注解@Name设置标签名称。
...
1. 完全由自己掌控整个表格的生成：参见link:#cus-policy-section[插件机制-指定模板策略]。
...
*插件机制是poi-tl的核心*，默认的五大內建模板语法是通过插件方式加载的，我们可以轻松的增加新的语法插件，也可以很轻松的处理任意模板标签。

插件的核心逻辑是通过poi和poi-tl提供的API操作word文档，实现插件就是实现自己的操作策略。

=== 新增自己的策略
你可以通过实现下面的接口实现新的渲染策略：
[source, java]
----
...
----
通过ElementTemplate获得当前模板位置，通过data获得渲染数据，通过XWPFTemplate获得Apache POI增强类NiceXWPFDocument，继而可以在当前模板位置插入段落，图片，表格等。
...
当我们有个模板标签为{{report}}，如果希望在这个位置做些不一样或者更复杂的事情，我们可以通过构建器设定模板的渲染策略：
[source, java]
