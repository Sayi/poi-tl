...
所有的标签都是以 `{{` 开头，以 `}}` 结尾，标签可以出现在任何位置，包括页眉，页脚，表格内部，文本框等等。poi-tl遵循**“所见即所得”**的设计，模板和标签的样式会被完全保留。
...
== 标签
poi-tl是一种无逻辑「logic-less」的模板引擎，没有复杂的控制结构和变量赋值，只有标签。标签由前后两个大括号组成， `{{title}}` 是标签， `{{?title}}` 也是标签， `title` 是这个标签的名称， `?` 标识了标签类型，接下来我们来看看有哪些默认标签类型(用户可以创建新的标签类型，这属于更高级的话题)。
...
put("welink", new HyperlinkTextRenderData("website", "http://deepoove.com"));
...
put("welink", Texts.of("website").link("http://deepoove.com").create());
...
[TIP]
====
现实需求中表格的布局和样式可能很复杂，此时默认表格样式将无法满足，可以尝试一些已有表格插件来解决，参见link:#plugin-list[可选插件列表]。

我们也可以编写插件，完全由自己生成整个表格，这个方案需要你熟悉Apache POI XWPFTable相关API，但是自由度最高：参见 link:#cus-policy-section[开发一个插件]。
====
...
区块对的循环功能可以很好的循环列表，并且保持有序列表编号有序。
====
...
| `HackLoopTableRenderPolicy` |循环表格行，下文会详细介绍 | link:#hack-loop-table[示例-表格行循环]
...
| `JSONRenderPolicy` |高亮显示JSON代码块 |link:#example-swagger[示例-Swagger文档]
...
| `TOCRenderPolicy` |实验功能：目录，打开文档时需要更新域 |
|===
...
NOTE: 源码参见 link:https://github.com/Sayi/poi-tl/blob/v1.8.2/src/test/java/com/deepoove/poi/tl/policy/HackLoopTableRenderPolicyTest.java[JUnit HackLoopTableRenderPolicyTest]，如果希望模板标签和循环行在同一行而不是在上一行，可以使用 `new HackLoopTableRenderPolicy(true)` 来构造插件。
...
Configure config = Configure.builder().bind("goods", policy).build(); <1>
...
这个示例主要用来展示link:#plugin-dynamic-table[DynamicTableRenderPolicy]的用法，货物明细和人工费仅仅是循环渲染表格行，使用link:#hack-loop-table[HackLoopTableRenderPolicy] 插件会更方便。
...
NOTE: 源码参见 link:https://github.com/Sayi/poi-tl/blob/v1.8.2/src/test/java/com/deepoove/poi/tl/example/AnimalExmample.java[JUnit AnimalExample]
