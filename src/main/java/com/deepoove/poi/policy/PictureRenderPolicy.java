...
public class PictureRenderPolicy implements RenderPolicy {
	...
	public void render(ElementTemplate eleTemplate, Object renderData, XWPFTemplate doc) {
		...
		try {
			...
			doc.getXWPFDocument().addPicture(blipId, doc.getXWPFDocument().getNextPicNameNumber(suggestFileType(pictureRenderData.getPath())), pictureRenderData.getWidth(), pictureRenderData.getHeight(), run);
			run.setText("", 0);
		} catch (Exception e) {
			logger.error("render picture docx error", e);
			run.setText(pictureRenderData.getNoExistShow(), 0);
		}
	}
	...
	private int suggestFileType(String imgFile) {
		...
		if (imgFile.endsWith(".emf"))
			...
			logger.error("Unsupported picture: " + imgFile + ". Expected emf|wmf|pict|jpeg|png|dib|gif|tiff|eps|bmp|wpg");
		}
		return format;
	}
